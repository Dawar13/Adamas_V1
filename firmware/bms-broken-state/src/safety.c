 ...
 * ###########################################################################
 *
 * This file belongs to firmware/bms-broken-state, which is a copy of
 * firmware/bms with ONE behaviour changed: the under-voltage rule has lost its
 * state qualifier and now fires in EVERY state. Nothing else in this tree
 * differs from the shipping BMS except the CMake project() name.
 *
 * WHAT IT DOES WRONG
 *   Rule 3 is specified as "< 60000 mV, RUNNING ONLY, latched". The three-line
 *   guard that returns false outside RUNNING has been deleted from
 *   bms_check_pack_undervolt() below. The limit, the strict comparison and the
 *   latch are untouched, so a pack collapsing under load faults exactly as the
 *   real one does -- which is the point: any scenario that only ever injects a
 *   low pack voltage while driving scores this binary as perfect.
 *
 *   What it gets wrong is the case the qualifier exists for. A pack that is
 *   merely flat while parked now latches a safety fault and opens the main
 *   contactor, and because that fault is terminal the vehicle cannot be
 *   brought up again. A charge cycle would have fixed it; instead it goes on
 *   a truck, and the workshop finds a battery that is simply empty.
 *
 * WHY IT EXISTS
 *   To prove Bench's discrimination rests on more than one file. bms-broken
 *   moves a threshold's BOUNDARY and bms-broken-latch moves a fault's
 *   RECOVERY; this one deletes a rule's STATE QUALIFIER, which can only be
 *   caught by a scenario that injects the same value twice in two different
 *   states and demands two different verdicts. A suite that tests the rule
 *   only in the state where it is supposed to fire cannot see this at all.
 *   See PHASE-2.md 7.2 and the EXPECTED-DIVERGENCE.yml beside this file, whose
 *   list of diverging tests was observed, not predicted.
 *
 ...
 *   thing this binary is fit for is failing the state-qualifier assertions on
 *   purpose.
 *
 * Everything below this banner is the real BMS's own commentary, unmodified --
 * including the paragraph explaining why a flat pack in standby must not
 * fault. It does here. That contradiction is the bug, preserved exactly as it
...
{
	/* ----------------------------------------------------------------------
	 * THE DEFECT. firmware/bms opens this function with
	 *
	 *     if (c->state != BMS_STATE_RUNNING) {
	 *             return false;
	 *     }
	 *
	 * and that guard has been deleted here, so the rule fires in every state
	 * -- including the standby case the paragraph above says it must not fire
	 * in. Nothing else changed: same limit, same strict comparison, same
	 * latch.
	 * -------------------------------------------------------------------- */

 ...
 * ###########################################################################
 *
 * This file belongs to firmware/bms-broken-precharge, which is a copy of
 * firmware/bms with ONE behaviour changed: during the precharge dwell the MAIN
 * CONTACTOR IS CLOSED FIRST and the precharge resistor is engaged afterwards.
 * The edit is the marked block in contactor_for() below. Nothing else in this
 * tree differs from the shipping BMS except the CMake project() name.
 *
 * WHAT IT DOES WRONG
 *   The specification is an ORDER, not a set of values. The precharge resistor
 *   limits inrush, and only once the bus has come up may the main contactor
 *   close. This build inverts those two inside the dwell: the main contactor
 *   closes onto a discharged bus, inrush welds the contacts shut, and a welded
 *   main contactor means the battery can never be disconnected again --
 *   including during a crash or a fire.
 *
 *   The dwell is the same length. The same two contactor states appear on the
 *   bus, at the same cadence, and the steady state at the end is CLOSED exactly
 *   as it should be. ONLY THEIR ORDER DIFFERS.
 *
 * WHY IT EXISTS
 *   To prove expect_order is needed. Every assertion V1 can write about this
 *   sequence passes against this binary: an expectation that PRECHARGE appears,
 *   and a later expectation that CLOSED appears, are both satisfied, because
 *   both values really do appear. V1 windows are sequential and cannot make two
 *   statements about the same interval, so "CLOSED did not appear before
 *   PRECHARGE" has no words in that vocabulary at all. See the
 *   EXPECTED-DIVERGENCE.yml beside this file, whose list of diverging tests was
 *   observed, not predicted.
 *
 * WHAT IT MUST NEVER BE
 *   It prints the same boot banner as the real BMS ("BMS ready") on purpose, so
 *   it is a drop-in swap for a divergence run. That also means the console
 ...
 * including the line in state_machine_run() explaining that a withdrawn request
 * falls back rather than "closing the main contactor onto nothing". Here the
 * main contactor closes onto exactly that, every time, by design. That
 * contradiction is the bug, preserved as it appears in the field.
 */
...
static uint8_t contactor_for(const bms_t *c)
{
	...
	case BMS_STATE_PRECHARGE:
		/* ------------------------------------------------------------------
		 * THE DEFECT. firmware/bms returns BMS_CONTACTOR_PRECHARGE here,
		 * unconditionally, for the whole of the dwell.
		 *
		 * Here the main contactor is closed for the first half of the dwell
		 * and the precharge resistor is engaged for the second half -- the two
		 * halves of the sequence, in the wrong order. Inrush therefore hits a
		 * discharged bus through a closed main contactor.
		 * ---------------------------------------------------------------- */
		return (c->state_ms < BMS_PRECHARGE_DWELL_MS / 2U)

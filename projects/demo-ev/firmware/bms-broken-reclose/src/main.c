 ...
 * ###########################################################################
 *
 * This file belongs to firmware/bms-broken-reclose, which is a copy of
 * firmware/bms with ONE behaviour changed: after a latched fault the main
 * contactor is held open for a fixed dwell AND THEN CLOSES AGAIN. The edit is
 * the marked block in contactor_for() below. Nothing else in this tree differs
 * from the shipping BMS except the CMake project() name.
 *
 * WHAT IT DOES WRONG
 *   The fault itself latches correctly. The state machine reaches FAULT and
 *   stays there; the fault code is published and keeps being published; the
 *   onset is identical to the real BMS in every observable way. What does not
 *   hold is the OUTPUT: 400 ms after the fault, the main contactor closes back
 *   onto a pack that has been declared unsafe.
 *
 *   On a vehicle that is a pack which re-energises the traction bus after a
 *   thermal or over-voltage event, with the fault still latched and still being
 *   reported. Every diagnostic says the pack is faulted; the contactor is shut.
 *
 * WHY IT EXISTS, AND WHY IT CARRIES NO MARKER
 *   It was built to justify expect_latched, and IT FAILED TO. That is what it
 *   now records, and it is kept for exactly that reason.
 *
 *   The prediction was that only a latch assertion could see this defect: an
 *   expectation RESOLVES ON ITS FIRST MATCH and stops looking, so "the
 *   contactor opens after the fault" is satisfied at the first OPEN frame and
 *   is blind to everything after it. That half held. Three controls were run
 *   against this binary in one run:
 *
 *     expect_can{OPEN}       PASS    blind, as predicted
 *     expect_no_can{CLOSED}  FAIL    the EXISTING verb catches it
 *     expect_always{OPEN}    FAIL    the other new verb catches it too
 *     expect_latched         BROKEN  set=1000400 lost=1400400
 *
 *   The prohibition was expected to raise a false alarm on the GOOD binary,
 *   because it must be armed at a moment the author guesses. It did not: the
 *   fault always propagates before the next 100 ms frame, so arming at the
 *   injection instant is robust rather than lucky. The prediction was wrong,
 *   and measuring it is what the control was for.
 *
 *   So the fixed-value form of expect_latched adds nothing here and did not
 *   ship. It moves to 3.3b in the VALUE-ANCHORED form -- "whatever fault code
 *   it first raised, it must keep reporting that same code" -- which is
 *   genuinely inexpressible today, because every matcher in this engine carries
 *   a compile-time constant and nothing can compare a frame against a value
 *   observed earlier in the same run.
 *
 *   THERE IS DELIBERATELY NO EXPECTED-DIVERGENCE.yml BESIDE THIS FILE. A marker
 *   is what makes a build a declared variant, and this one is not offered to
 *   the gate: no shipped verb rests on it. Adding a marker later is how 3.3b
 *   begins, once there is a verb whose absence it can prove.
 *
 ...
 *
 * Everything below this banner is the real BMS commentary, unmodified --
 * including the line that calls FAULT terminal and says the contactor opens
 * there. It opens, and then it shuts. That contradiction is the bug.
 */
...
static uint8_t contactor_for(const bms_t *c)
{
	...
	case BMS_STATE_FAULT:
		/* ------------------------------------------------------------------
		 * THE DEFECT. firmware/bms returns BMS_CONTACTOR_OPEN here, with no
		 * dwell and no condition: FAULT is terminal and the contactor stays
		 * open for as long as the node is powered.
		 *
		 * Here it is held open for 400 ms and then closes again, re-energising
		 * the traction bus while the fault is still latched and still being
		 * published.
		 * ---------------------------------------------------------------- */
		return (c->state_ms < 400U) ? BMS_CONTACTOR_OPEN : BMS_CONTACTOR_CLOSED;
	case BMS_STATE_INIT:
	case BMS_STATE_STANDBY:
	default:

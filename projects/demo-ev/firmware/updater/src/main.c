/*
 * updater — a device that installs a firmware image into its own flash, and
 * checks on every boot whether the image it holds is usable.
 *
...
static void install_image(void)
{
	...
	/* THE HEADER LAST. Until this single word is programmed the image is
	 * not claimed to be complete, and the validator will refuse it. */

 ...
 * ###########################################################################
 *
 * This file belongs to firmware/bms-broken, which is a byte-for-byte copy of
 * firmware/bms with ONE character changed: the over-temperature comparison on
 * line ~90 below reads `>=` where the real BMS reads `>`. Nothing else in this
 * tree differs from the shipping BMS except the CMake project() name.
 *
 * WHY IT EXISTS
 *   To prove Bench genuinely executes the binary under test rather than
 *   replaying a recording. Same platform, same scenarios, a different binary,
 *   a different answer. `>` versus `>=` is a real bug class: a one-character
 *   mistake that manual testing does not see and a boundary sweep catches
 *   immediately. See PROJECT.md §3 and PHASE-1.md §1.6.
 *
 * WHAT IT DOES WRONG
 *   BMS_OVERTEMP_LIMIT_dC is 550 and the specification says 550 dC (55.0 C) is
 *   LEGAL — only 551 and above may fault. This build faults at exactly 550, so
 *   it opens the main contactor on a pack that is within its rated envelope.
 *   On a vehicle that is an unnecessary shutdown at the top of the normal
 *   operating range.
 *
 * WHAT IT MUST NEVER BE
 *   It prints the same boot banner as the real BMS ("BMS ready") on purpose,
 *   so it is a drop-in swap for the demo. That also means the console cannot
 *   tell you which binary is loaded. It must never be presented as a working
 *   ECU, never flashed to hardware, never used as the baseline for anything,
 *   and never referenced by network.yml as the BMS node. The only thing this
 *   binary is fit for is failing the boundary scenarios on purpose.
 *
 * Everything below this banner is the real BMS's own commentary, unmodified —
 * including the line that says this comparison is strictly greater. It is not.
 * That contradiction is the bug, preserved exactly as it appears in the field.
 */

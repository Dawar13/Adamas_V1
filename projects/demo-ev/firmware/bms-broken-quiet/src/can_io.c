 ...
 * ###########################################################################
 *
 * This file belongs to firmware/bms-broken-quiet, which is a copy of
 * firmware/bms with ONE behaviour changed: bms_limits (0x602) IS NEVER
 * TRANSMITTED. The edit is the marked block in bms_can_service_tx() below.
 * Nothing else in this tree differs from the shipping BMS except the CMake
 * project() name.
 *
 * WHAT IT DOES WRONG
 *   The cadence timer still advances -- so every other frame keeps its exact
 *   phase -- but its verdict is discarded and the limits frame is never sent.
 *   The VCU therefore never learns the pack current limits and never learns the
 *   contactor state. It is the shape a publisher takes when it is disabled
 *   during debugging and never restored.
 *
 * WHY IT EXISTS
 *   To prove expect_always is needed, and to demonstrate a live hole in the
 *   current vocabulary. expect_no_can is judged "FAIL if hit else PASS": a
 *   prohibition on a frame that never arrives is reported as honoured. Against
 *   this binary, "the main contactor was never closed during startup" -- a
 *   safety statement -- is GREEN, and it is green precisely because the device
 *   went silent and nothing could be observed at all.
 *
 *   An invariant is the other half of that sentence: every observation
 *   satisfied it, AND there were observations. Zero samples is a failure, not a
 *   pass. See the EXPECTED-DIVERGENCE.yml beside this file, whose list of
 *   diverging tests was observed, not predicted.
 *
...
void bms_can_service_tx(bms_t *c)
{
	...
	/* ----------------------------------------------------------------------
	 * THE DEFECT. firmware/bms assigns this timer's verdict to limits_due.
	 *
	 * Here the timer is still advanced -- so every other cadence keeps the
	 * exact phase it would otherwise have had -- and its answer is thrown
	 * away. bms_limits (0x602) is never transmitted at all.
	 * ------------------------------------------------------------------- */
	(void)tx_timer_due(&tx_acc_limits_ms, BMS_TX_PERIOD_LIMITS_MS);
	limits_due = false;
